...
if options.year == 19:
    if options.scheme == "pT_eta":
        ttree.Add(f"{options.input}/{options.meson}__{options.year}_{options.size}_bin{options.bin}.root")
...
if binned:
    with LHCbStyle():
        ...
        print("Saving plots")
        if options.scheme != "total":
            c.SaveAs(f"{options.path}/{options.meson}_{options.year}_{options.size}_bin{options.bin}_fit_ANA.pdf")
            if Failed == 0:
                pull_canvas.SaveAs(f"{options.path}/{options.meson}_{options.year}_{options.size}_bin{options.bin}_fit_pulls.pdf")
            file = open(f"{options.path}/yields_{options.meson}_{options.year}_{options.size}_bin{options.bin}.txt", "w+")
            if Failed == 0:
            ...
            c.SaveAs(f"{options.path}/{options.meson}_{options.year}_{options.size}_fit_ANA.pdf")
            if Failed == 0:
                pull_canvas.SaveAs(f"{options.path}/{options.meson}_{options.year}_{options.size}_fit_pulls.pdf")
            file = open(f"{options.path}/yields_{options.meson}_{options.year}_{options.size}.txt", "w+")
    ...
    chi2, pull_mean, pull_std = plot(D0_M, unbinned_data, model, nbins=numbins, setlogy=False, save_to=f'{options.path}/{options.meson}__{options.year}_{options.size}', plot_type=f"Total 20{options.year}.title()", meson=options.meson)
    # Write out results
    file = open(f"{options.path}/yields_{options.meson}_{options.year}_{options.size}.txt", "w+")
